...
class UpgradeButton: Sprite {
    ...
    init(name: String, x: CGFloat, y: CGFloat) {
        super.init(named: name, x: x, y: y)
        //self.startAtTop = startAtTop
        self.setScale(2.0)
        //set physics properties
        self.physicsBody = SKPhysicsBody(rectangleOfSize: CGSize(width: 150, height: 150))
